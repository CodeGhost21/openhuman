...
const OAuthProviderButton = ({
  ...
  const handleOAuthLogin = async () => {
    ...
    try {
      // Desktop only: wait for the embedded core to be reachable before we
      // hand the user off to the system browser — otherwise the OAuth deep
      // link returns and `consume_login_token` races a not-yet-listening
      // core (issue #1689).
      if (isTauri()) {
